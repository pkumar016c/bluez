...
#include "inventory.h"
#define SOCKET_PATH "/tmp/obs.socket"
    int server_socket, client_socket;
int totalDataReceived = 0;
volatile int stop_socket = 1;
pthread_mutex_t dataMutex = PTHREAD_MUTEX_INITIALIZER;
#define DEVICE_NAME "OBS-truck"
...
int data_ready =0;
#define APP_UUID "12345678-1234-5678-1234-56789abcdef0" // Your custom UUID
const char *custom_obs_uuid="fd8eddd0-99fa-b4b8-bc48-e6177e96f741";
const char *custom_obs_read_uuid="fd8eddd1-99fa-b4b8-bc48-e6177e96f741";
const char *custom_obs_read_notify="fd8eddd2-99fa-b4b8-bc48-e6177e96f741";
// Define UUIDs for your characteristics
#define UUID_DOOR_STATUS 0xDDD0
#define UUID_READ_INVENTORY 0xDDD1
#define BLE_UUID_OUR_BASE_UUID  {0x41, 0xF7, 0x96, 0x7E, 0x17, 0xE6, 0x48, 0xBC, 0xB8, 0xB4, 0xFA, 0x99, 0x89, 0xE5, 0x8E, 0xFD}
#define BLE_UUID_OUR_BASE_UUID1  {0x42, 0xF7, 0x96, 0x7E, 0x17, 0xE6, 0x48, 0xBC, 0xB8, 0xB4, 0xFA, 0x99, 0x89, 0xE5, 0x8E, 0xFD}
#define ATT_CID 4
#define MAC_ADDRESS_LENGTH 6
struct gatt_db_attribute *obs;
#define PRLOG(...) \
	do { \
		printf(__VA_ARGS__); \
		print_prompt(); \
...
uint8_t flattened_macids[4096]; // 6 bytes for each MAC address
size_t received_macs_len = 0;
static void data_to_send(){
    int flattened_index = 0;

    for (int i = 0; i < MAC_ADDRESS_COUNT; i++) {
        char *mac = inventory[i];
        char *token = strtok(mac, ":"); // Split the MAC address into components

        while (token != NULL) {
            uint8_t componentInt = (uint8_t)strtol(token, NULL, 16);
            flattened_macids[flattened_index] = componentInt;
            flattened_index++;
...
uint8_t values[MAC_ADDRESS_COUNT][6]; // 6 bytes for each MAC address
/*
static void data_to_send(){
  for (int i = 0; i < MAC_ADDRESS_COUNT; i++) {
    char *mac = inventory[i];
    ...
    while (token != NULL) {
      uint8_t componentInt = (uint8_t)strtol(token, NULL, 16);
      values[i][byteIndex] = componentInt;
      byteIndex++;
    ...
    for (int j = 0; j < 6; j++) {
      printf("%02X", values[i][j]); // Print each byte as a 2-digit hexadecimal number
      if (j < 5) {
        printf(":");
      }
    }
...
static bool manifest_ready_cb(void *user_data)
{
struct server *server = user_data;
//uint8_t pdu;
	uint8_t pdu[4];
...
void macToInteger(const char* mac, int* result) {
    // Split the MAC address into its individual components
    uint8_t macBytes[6];
    if (sscanf(mac, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &macBytes[0], &macBytes[1], &macBytes[2], &macBytes[3], &macBytes[4], &macBytes[5]) == 6) {
        // Convert the MAC address bytes into a single integer value
        *result = (macBytes[0] << 40) | (macBytes[1] << 32) | (macBytes[2] << 24) | (macBytes[3] << 16) | (macBytes[4] << 8) | macBytes[5];
    } else {
        // Handle parsing error (invalid MAC address format)
        *result = -1; // Or use another appropriate error value
    }
}

static void read_inventory_read_cb_second(struct gatt_db_attribute *attrib,
                unsigned int id, uint16_t offset,
                uint8_t opcode, struct bt_att *att,
                void *user_data)
{
	printf("OBS read Callback\n");

	//data_to_send();
	printf("size of returned value = %d\n",received_macs_len);

	/*gatt_db_attribute_write(obs, 0, (void *) &values, 12,
							BT_ATT_OP_WRITE_REQ,
							NULL, confirm_write,
							NULL);*/
	/*uint8_t value[30];
	    for (int i = 0; i < 30; i++) {
        value[i] =i+3; // Generate a random number between 0 and 255
		printf("%d ", value[i]);
    }*/
    int batch_size = 18;
    int total_mac_count = MAC_ADDRESS_COUNT * MAC_ADDRESS_LENGTH;
	int start = 0;
    int len = (received_macs_len - 504);
	printf("length of second payload = %d \n",len);
	gatt_db_attribute_read_result(attrib, id, 0, &flattened_macids[504], len);
	//start = start+504;
	/*
    for (int i = 0; i < total_mac_count; i += batch_size) {
        // Check if there are enough bytes remaining to send a full batch
        int remaining_bytes = total_mac_count - i;
        int bytes_to_send = (remaining_bytes < batch_size) ? remaining_bytes : batch_size;

        // Send the batch of data (flattened_macids[i] is the starting address of the batch)
        int res = gatt_db_attribute_read_result(attrib, id, 0, &flattened_macids[i], bytes_to_send);

        // Check res for success/failure handling
        if (res != 0) {
			printf("error sending read data\n");
        }
    }*/
...
static void read_inventory_read_cb(struct gatt_db_attribute *attrib,
                unsigned int id, uint16_t offset,
                uint8_t opcode, struct bt_att *att,
                void *user_data)
{
    ...
    int total_mac_count = MAC_ADDRESS_COUNT * MAC_ADDRESS_LENGTH;
	int start = 0;
    int len = (504 < received_macs_len)? 504 :received_macs_len;
	printf("length of first payload = %d \n",len);
	gatt_db_attribute_read_result(attrib, id, 0, &flattened_macids[start], len);
	//start = start+504;
	/*
    for (int i = 0; i < total_mac_count; i += batch_size) {
        // Check if there are enough bytes remaining to send a full batch
...
static void door_status_read_cb(struct gatt_db_attribute *attrib,
                unsigned int id, uint16_t offset,
                uint8_t opcode, struct bt_att *att,
                void *user_data)
{

    //  gatt_db_attribute_read_result to send the values back
}
static void send_data_ready_notification(struct server *server)
{
	printf("called send_data_ready_notification \n");
	timeout_add(1000, manifest_ready_cb, server, NULL);
}


static void obs_mfst_ccc_write_cb(struct gatt_db_attribute *attrib,
					unsigned int id, uint16_t offset,
					const uint8_t *value, size_t len,
					uint8_t opcode, struct bt_att *att,
					void *user_data)
{
	...
	printf("Data ready notification enabled \n");
	send_data_ready_notification(server);
done:
	gatt_db_attribute_write_result(attrib, id, ecode);
}
...
static void populate_hr_service(struct server *server)
{
	...
	hr_msrmt = gatt_db_service_add_characteristic(service, &uuid,
						...
						BT_GATT_CHRC_PROP_WRITE |
					 	BT_GATT_CHRC_PROP_NOTIFY,
						NULL, mcs_media_cp_write, NULL);
	server->hr_msrmt_handle = gatt_db_attribute_get_handle(hr_msrmt);

	bt_uuid16_create(&uuid, GATT_CLIENT_CHARAC_CFG_UUID);
	gatt_db_service_add_descriptor(service, &uuid,
					BT_ATT_PERM_READ | BT_ATT_PERM_WRITE,
					read_inventory_read_cb,
					obs_mfst_ccc_write_cb, server);
	
    bt_uuid16_create(&uuid, UUID_HEART_RATE_BODY);
	body = gatt_db_service_add_characteristic(service, &uuid,
						BT_ATT_PERM_READ,
						BT_GATT_CHRC_PROP_READ,
						read_inventory_read_cb_second, obs_mfst_ccc_write_cb, server);

	if (server->hr_visible)
		gatt_db_service_set_active(service, true);
}

static void populate_obs_service(struct server *server)
{
	bt_uuid_t uuid;
	struct gatt_db_attribute *service, *obs;
	uint128_t uuid_value;

	/* Add OBS Service */
	memcpy(uuid_value.data, custom_obs_uuid, 16);
	bt_uuid128_create(&uuid, uuid_value);
	service = gatt_db_add_service(server->db, &uuid, true, 8);

	/* Data ready notify Characteristic */
	memcpy(uuid_value.data, custom_obs_read_notify, 16);
	bt_uuid128_create(&uuid, uuid_value);
	obs = gatt_db_service_add_characteristic(service, &uuid,
						BT_ATT_PERM_READ,
						BT_GATT_CHRC_PROP_READ,
						read_inventory_read_cb, NULL, NULL);
	server->obs_handle = gatt_db_attribute_get_handle(obs);
/*
	memcpy(uuid_value.data, custom_obs_read_uuid, 16);
	bt_uuid128_create(&uuid, uuid_value);
	gatt_db_service_add_descriptor(service, &uuid,
							BT_ATT_PERM_READ | BT_ATT_PERM_WRITE,
							read_inventory_read_cb, 
							read_inventory_write_cb, server);
	server->obs_mfst_handle = gatt_db_attribute_get_handle(obs);
...
{
	bt_uuid_t uuid;
	struct gatt_db_attribute  *service, *obs, *obs_mfst;
	uint8_t body_loc = 1;  /* "Chest" */
	uint128_t uuid_value;
	...
	memcpy(uuid_value.data, custom_obs_uuid, 16);
	bt_uuid128_create(&uuid, uuid_value);
	obs = gatt_db_add_service(server->db, &uuid, true, 8);

	server->obs_handle = gatt_db_attribute_get_handle(obs);

	obs_mfst = gatt_db_service_add_characteristic(obs, &uuid,
						BT_ATT_PERM_NONE,
						BT_GATT_CHRC_PROP_NOTIFY,
						NULL, NULL, NULL);
	server->obs_mfst_handle = gatt_db_attribute_get_handle(obs_mfst);
	memcpy(uuid_value.data, custom_obs_read_uuid, 16);
	bt_uuid128_create(&uuid, uuid_value);
	gatt_db_service_add_descriptor(service, &uuid,
					BT_ATT_PERM_READ | BT_ATT_PERM_WRITE,
					read_inventory_read_cb,
					obs_mfst_ccc_write_cb, obs);
	gatt_db_service_set_active(service, true);
}
#endif
...
//	populate_gatt_service(server);
	...
	populate_obs_service(server);
}
int uuid_string_to_bytes(const char *uuid_string, uint8_t *bytes) {
    // Perform the conversion from the UUID string to bytes here.
    // You'll need to parse the string and fill the bytes array.

    // For example, you can use sscanf or a custom parsing function.
    ...
    return (result == 16) ? 0 : -1; // Return an error code if parsing fails
}
							...
							uint8_t src_type)
{
	...
	int hci_le_set_advertising_data_cp_size = 31;
	//uint8_t adv_data[hci_le_set_advertising_data_cp_size];
	char ba[18];
/*
	 // Set BLE advertisement data
  memset(&adv_data, 0, sizeof(adv_data));
  adv_data[0] = 0x02; // Length of the data
  adv_data[1] = 0x01; // Flags
  adv_data[2] = 0x06; // General Discoverable, BR/EDR not supported
  adv_data[3] = 0x09; // Length of the name
  adv_data[4] = 0x09; // Name
  memcpy(&adv_data[5], DEVICE_NAME, strlen(DEVICE_NAME));
	size_t adv_data_len = sizeof(adv_data);
  if (hci_le_set_advertising_data(sock, adv_data_len, adv_data) < 0) {
    perror("Error setting advertisement data");
    return -1;
  }
...
uint8_t adv_data[] = {
    ...
    // Complete local name (replace with your device name)
    0x0C, 0x09, 'O', 'B', 'S', ' ', ' ', 'T', 'R', 'U', 'C', 'K', ' ',

    // Manufacturer-specific data (for custom data like "Door Status = Closed")
    0x0E, 0xFF, 0x00, 0x01, 'D', 'o', 'o', 'r', ' ', 'S', 't', 'a', 't', 'u', 's', '=', 'C', 'l', 'o', 's', 'e', 'd'
};

// Set the advertising data
//bt_le_set_advertising(adv_data, sizeof(adv_data));
//bt_start_advertising();
	uint8_t att_response[31]; // Define the response buffer

	// Populate the response buffer with your advertising data
	memcpy(att_response, adv_data, sizeof(adv_data));
	...
	int status = system("sudo config --write APP_CONNECTED 1");
...
static void signal_cb(int signum, void *user_data)
{
	...
	case SIGINT:
	case SIGTERM:
		system("sudo config --write APP_CONNECTED 0");
		mainloop_quit();
		break;
	default:
		break;
...
void* handleClient(void* arg) {
    ...
    while (stop_socket) {
        ...
        if (bytesReceived > 0) {
            ...
            // Split the received data into MAC addresses
            char *token = strtok(buffer, ":");
          	while (token != NULL) {
                // Convert the received MAC address to u_int8_t
                u_int8_t componentInt = (u_int8_t)strtol(token, NULL, 16);
				printf("%02X",componentInt);
                // Append the component to the received_macs array
                flattened_macids[received_macs_len++] = componentInt;
                // Get the next MAC address component
                token = strtok(NULL, ":");
            }
		send(client_socket, "ACK sending it to App", 21, 0);
		data_ready=1;
        }
...
int main(int argc, char *argv[])
{
	...
	int status = system("sudo rm -rf /tmp/obs.socket");
	status = system("sudo config --write APP_CONNECTED 0");
	...
	if (pthread_create(&mainThread, NULL, handleClient, NULL) != 0) {
...
/*
	...
	mainloop_add_fd(fileno(stdin),
				EPOLLIN | EPOLLRDHUP | EPOLLHUP | EPOLLERR,
				prompt_read_cb, server, NULL);
